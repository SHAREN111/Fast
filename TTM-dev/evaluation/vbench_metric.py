...
if __name__ == '__main__':
    ...
    raw_dir = f"{base_dir}/evaluation/{arg.exp_name}"
    json_dir = f"{base_dir}/evaluation/results/{arg.exp_name}"

    # avg_time,avg_mem = get_avg

    cal_score(raw_dir, json_dir, name='all', 
              unpruned_videos_path='/mnt/dolphinfs/hdd_pool/docker/user/hadoop-mtsearch-assistant/ai-search/dongchengqi/InfinityStar/TTM-dev/evaluation/raw', 
              time=0, memory=0)

    videos_path = f"{base_dir}/evaluation/{arg.exp_name}"
    mode = os.path.basename(videos_path)
    name = "all"

    result_dir = f"{base_dir}/evaluation/results/{arg.exp_name}"
    os.makedirs(result_dir, exist_ok=True)
    ...
    my_VBench = VBench(
        'cuda',
        f"{base_dir}/evaluation/VBench_rewrited_prompt.json",
        result_dir
