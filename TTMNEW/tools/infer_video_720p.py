...
from evaluation.visual_metric import cal_vis_metric
...
def perform_inference(pipe, data, args):
    ...
    with torch.cuda.amp.autocast(enabled=True, dtype=torch.bfloat16, cache_enabled=True), torch.no_grad():
        generated_image, _, _ = gen_one_example(
            pipe.infinity,
            pipe.vae,
            pipe.text_tokenizer,
            pipe.text_encoder,
...
if __name__ == '__main__':
    ...
    # infer args
    args = Args()
    args.mode = 'raw'    #'raw' #'fastvar','ttm','sparsevar'
    args.pn='0.90M'
    args.fps=16
    ...
    prompts = [
    "A confident chef preparing a dish in a busy kitchen, cinematic lighting, rich steam effects, ultra-detailed, realistic motion",
    "A woman jogging along a lakeside at sunrise, soft golden-hour lighting, natural camera shake, highly detailed environment, ultra-realistic",
    "A scientist observing a glowing experiment in a dark lab, dramatic rim lighting, intricate reflections, slow-motion particles, cinematic realism",
    "A stylish man walking through a neon-lit city street at night, depth-of-field blur, reflective surfaces, realistic atmospheric haze, ultra HD details",
    "A child chasing bubbles in a garden, soft natural light, shallow depth of field, vivid colors, smooth dynamic motion, hyper-realistic",
    "A barista pouring latte art in a cozy café, warm cinematic lighting, close-up macro textures, gentle steam motion, highly detailed realism",
    "A traveler standing on a cliff with wind blowing their coat, dramatic clouds, epic wide-angle shot, highly textured scenery, ultra-realistic",
    "A dancer performing in an abandoned industrial hall, volumetric lighting through windows, dust particles, fluid movement, photorealistic details",
    "An elderly man reading under a tree at sunset, warm soft light, gently moving leaves, highly detailed skin textures, realistic atmosphere",
    "A snowboarder carving through fresh powder snow on a mountain, dynamic action shot, cold crisp lighting, snow particles, ultra-realistic motion",
    "A drone flies low over a dense tropical rainforest, mist rising between the trees at sunrise.",
    "A young woman runs through a busy night market filled with colorful lights and moving crowds.",
    "A golden retriever jumps into a lake in slow motion, splashing water everywhere.",
    "A futuristic train glides through a neon-lit cyberpunk city during heavy rain.",
    "A small robot carefully waters plants in a quiet greenhouse, sunlight filtering through the glass.",
    "A group of penguins waddles across an icy tundra while snow gently falls.",
    "A flock of birds takes off from a quiet lakeside, ripples spreading across the water.",
    "A horse gallops across a wide desert plain, dust swirling beneath its hooves.",
    "A diver swims through a coral reef surrounded by schools of colorful fish.",
    "A street guitarist performs in a busy plaza while pedestrians walk past in the background."
]
    for idx, prompt in enumerate(prompts):
        ...
        save_dir = f'/mnt/dolphinfs/hdd_pool/docker/user/hadoop-mtsearch-assistant/ai-search/dongchengqi/InfinityStar/TTM-dev/visual/raw'#new_{args.mode}
        gen_video_path = osp.join(save_dir, f'demo{idx}.mp4')
        save_video(output_dict['output'], fps=args.fps, save_filepath=gen_video_path)
                
        print(f"Video genernation done: {gen_video_path=}")
    cal_vis_metric('/mnt/dolphinfs/hdd_pool/docker/user/hadoop-mtsearch-assistant/ai-search/dongchengqi/InfinityStar/TTM-dev/visual/raw', save_dir)
