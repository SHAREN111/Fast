...
def encode_prompt(t5_path, text_tokenizer, text_encoder, prompt, enable_positive_prompt=False, low_vram_mode=False):
    ...
    if 'flan-t5' in t5_path:
        ...
        # ⭐⭐ 加这几行 ⭐⭐
        # raw_tokens = text_tokenizer.tokenize(prompt)
        # print("=== Tokenized ===")
        # print(raw_tokens)
        ##############################
...
def gen_one_example(
    ...
    print(f"cost: {time.time() - sstt}, infinity cost={time.time() - stt}, memory {peak_mem} GB")
    img = img_list[0]
    return img, pred_multi_scale_bit_labels, peak_mem
